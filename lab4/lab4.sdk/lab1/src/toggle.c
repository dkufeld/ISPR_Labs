...
int main (){
  ...
  int xStatus;
  u32 uPinDirectionEMIO = 0x0; //wofür??
  u32 uPinDirection = 0x1; //wofür??
  ...
  XGpioPs_SetDirectionPin(&psGpioInstancePtr, fsm_led.fsm_gpio.pin,uPinDirection);
  XGpioPs_SetOutputEnablePin(&psGpioInstancePtr, fsm_led.fsm_gpio.pin,1);
  //EMIO PIN Setting to Input port
  XGpioPs_SetDirectionPin(&psGpioInstancePtr,
		  fsm_but.fsm_gpio.pin,uPinDirectionEMIO);
  XGpioPs_SetOutputEnablePin(&psGpioInstancePtr, fsm_but.fsm_gpio.pin,0);
  ...
  while (1) {
    fsm_fire (&fsm_but);
    fsm_fire (&fsm_led);
    for (i=0; i<99999; i++); // delay loop => should be done with timer
  }
}
